...
ana_timestamp = '20191126.171630'
ana_index = 4
ana_fomnames = ['Emin.Vrhe', 'Emax.Vrhe', 'Jmin.mAcm2', 'Jmax.mAcm2']
ana_csvpath = glob("%s/%s*/ana__%i*.csv" %(ANALYSIS_DIR, ana_timestamp, ana_index))[0]
...
for fomname in ana_fomnames:
    ...
    p = PatchCollection(patches, cmap=mpl.cm.viridis)
    p.set_array(fom)
    clim = p.get_clim()
    if 'Jmin' in fomname:
        p.set_clim([clim[0], 0])
    ...
    f_setlim(ax)
    addlabels(ax)
    plt.savefig('%s_heatmap.eps' %fomname)
    plt.savefig('%s_heatmap.png' %fomname)
