...
def scatter_4axes(comps, fom, stpl, cb=False, cbrect=(.9, .3, .02, .4), cblabel='', **kwargs):
    for i, stp in zip([3, 2, 1, 0], stpl):
        ...
        if len(inds)>0:
            stp.scatter(comps[inds][:, ci], c=fom[inds], s=36, **kwargs)
	if cb:
...
def plotbinarylines_quat(ax, comps, fom, ellabels=['A', 'B', 'C', 'D'], legloc=9, **kwargs):
    cb=comps>.001
    ...
    try:
        ax.legend(loc=legloc, ncol=6, mode="expand")
    except:
        pass
